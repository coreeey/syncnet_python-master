...
parser = argparse.ArgumentParser(description = "syncnet Demo or Columnbia ASD Evaluation")

parser.add_argument('--videoName',             type=str, default="test",   help='Demo video name')
parser.add_argument('--videoFolder',           type=str, default="demo",  help='Path for inputs, tmps and outputs')
parser.add_argument('--pretrainModel',         type=str, default="syncnet_v2.model",   help='Path for the pretrained TalkNet model')
	...
	for file in tqdm.tqdm(files, total = len(files)):
		...
		while video.isOpened():
			...
			if ret == True:
				...
				face = cv2.resize(frames, (224, 224))
		...
		sample_rate, audio = wavfile.read(os.path.join(args.pycropPath, fileName + '.wav'))
		...
		for i in range(0, lastframe, args.batch_size):
			...
			im_out = s.__S__.forward_lip(im_in);
			im_feat.append(im_out.data.cpu())

			cc_batch = [cct[:, :, :, vframe * 4:vframe * 4 + 20] for vframe in
						range(i, min(lastframe, i + args.batch_size))]
			cc_in = torch.cat(cc_batch, 0)
			cc_out = s.__S__.forward_aud(cc_in)
		...
		allScores.append(np.array(fconfm))
	return allScores
